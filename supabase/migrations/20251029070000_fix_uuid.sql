-- Fix UUID generation to use gen_random_uuid() instead of uuid_generate_v4()
-- This is the recommended approach for modern Supabase/PostgreSQL

-- Drop existing tables if they exist
DROP TABLE IF EXISTS public.workout_exercises CASCADE;
DROP TABLE IF EXISTS public.tracking_entries CASCADE;
DROP TABLE IF EXISTS public.tracking_plans CASCADE;
DROP TABLE IF EXISTS public.scheduled_workouts CASCADE;
DROP TABLE IF EXISTS public.workouts CASCADE;
DROP TABLE IF EXISTS public.exercises CASCADE;
DROP TABLE IF EXISTS public.profiles CASCADE;

-- Enable UUID extension (using the built-in pgcrypto for gen_random_uuid)
